...
def generar_imagen_flujo(
    ...
    """
    Genera imagen JPEG del análisis de flujo:
    - Fondo: hillshade del MDT
    - Azul claro: pendiente baja (acumulación de agua)
    - Azul oscuro: cauces (alta acumulación)
    - Rojo: pendiente alta
    """
    h, w = dem.shape
    rgb = np.zeros((h, w, 3), dtype=np.uint8)

    # 1. Hillshade como fondo
    dem_norm = dem.copy()
    dem_valid = dem_norm[~np.isnan(dem_norm)]
    if len(dem_valid) > 0:
        dmin, dmax = np.nanmin(dem_norm), np.nanmax(dem_norm)
        if dmax > dmin:
            dem_norm = (dem_norm - dmin) / (dmax - dmin)
        else:
            dem_norm = np.zeros_like(dem_norm)
    dem_norm = np.nan_to_num(dem_norm, nan=0.5)
    shade = (dem_norm * 120 + 40).astype(np.uint8)
    rgb[:,:,0] = shade
    rgb[:,:,1] = shade
    rgb[:,:,2] = shade

    # 2. Zonas de acumulación alta = cauces (azul intenso)
    acc_norm = np.log1p(acumulacion.astype(np.float32))
    acc_max = acc_norm.max()
    if acc_max > 0:
        acc_norm = acc_norm / acc_max

    # Umbral para cauces (top 5% de acumulación)
    umbral_cauce = np.percentile(acc_norm, 95)
    cauces = acc_norm >= umbral_cauce

    # Umbral para zonas de acumulación media (top 20%)
    umbral_acum = np.percentile(acc_norm, 80)
    acum_media = (acc_norm >= umbral_acum) & ~cauces

    # 3. Pendiente alta = rojo (erosión)
    pend_alta = pendiente > 15  # >15 grados

    # Colorear
    rgb[acum_media, 0] = 100
    rgb[acum_media, 1] = 180
    rgb[acum_media, 2] = 220  # Azul claro

    rgb[cauces, 0] = 0
    rgb[cauces, 1] = 80
    rgb[cauces, 2] = 200   # Azul oscuro (cauces)

    rgb[pend_alta & ~cauces, 0] = 200
    rgb[pend_alta & ~cauces, 1] = 80
    rgb[pend_alta & ~cauces, 2] = 60   # Rojo (pendiente alta)

    # Aplicar máscara de parcela
    if mask_arr is not None:
        rgb[~mask_arr] = [15, 15, 15]
...
async def analizar_flujo(
    ...
    # Generar imagen
    img_bytes = generar_imagen_flujo(dem, acumulacion, pendiente, mask_arr)
